...
class Game {
  ...
  skip() {
    ...
    console.log('[Questino skipped]');
    return this.sendNewQuestion();
  }

  send(text) {
    return this.bot.sendChatAction(this.chat.id, 'typing')
      .then(() => new Promise((resolve) => setTimeout(resolve, MESSAGE_TYPING_DELAY)))
  ...
  generateNextHint(answer) {
    this.currentHint = hintsGenerator.generateNextHint(this.currentHint, answer, MAX_HINTS);
  ...
  runHintsSequence(question) {
    ...
    const sendHint = () => {
      ...
      this.send(`Подсказка: ${this.generateNextHint(question.answer)}`);
